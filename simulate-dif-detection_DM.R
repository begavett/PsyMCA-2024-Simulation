...
load("~/Dropbox/PsyMCA2024_Data_Sharing_HCAP/HCAP_harmonized_data_PsyMCA-v5.rdata")
...
## DM shoould be: rescale the fmem score for HAALSI: mean = - 0.90, sd = 0.75
...
# # DM: Adding depression as a predictor of cog will change the residual variance
# # DM folow-up - not by much
...
# DM: Negative kurtosis values were not achieved in simulation
...
# 
# # DM Only 1 item has negative skew. This seems like the more common and concerning 
# #   problem inneueopsychology
...
# DM: x1 has a response category for <= 0
eq_int_scheme
...
sim_dif_detection <- function(iter, 
  ...
  # # DM: does slice randomly select, or take first n 
  set.seed(iter)
  ...
  for(suffix in suffixes) {
    ...
    if(nrow(dif_mod) > 0 & ifelse(nrow(dif_mod) > 0, any(dif_mod$converged), FALSE)) {
        ...
        # DM: should check distribution of factor score to verify that the match
...
# test1 <- sim_dif_detection(iter = 5)
